...
class AnoseekInference:
    ...
    def predict_df(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        IPs with fewer than 3 flows are classified as Benign (too little signal).
        """
        if df.empty:
        ...
        for ip in pd.unique(src_ips):
            ...
            if len(rows) < 3:
                confidences[idx] = 1.0
                continue  # pred_classes stays 0 (Benign)
    ...
    def ingest_live_flow(self, flow: dict) -> dict | None:
        """
        Feed one live flow dict. Returns a classification result each time the rolling
        buffer for that source IP is full (SEQ_LENGTH rows); returns None until then.
        """
        ...
        self._live_buffers[src_ip].append(self._scale_features(pd.DataFrame([flow]))[0])
